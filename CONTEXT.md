...
### Local Trust Mode
Tools werden im MVP als lokale Subprozesse ausgeführt, nicht als vollständig isolierte Sandbox. Dieses Modell wird sichtbar als **Local Trust Mode** bezeichnet. Es gelten trotzdem Timeout-, Output-, Prozess-, Environment- und Workspace-Limits. Deklarierte Schreibverzeichnisse werden als kooperativer Tool-Vertrag (`HARNESSFORGE_ALLOWED_WRITE_DIRS`) an das Skript übergeben und vor der Ausführung validiert; ein beliebiges Skript kann diese Variable im MVP jedoch nicht technisch erzwingen. Für untrusted Skripte oder harte Schreibisolation ist eine externe OS-Sandbox wie Docker/Podman erforderlich; HarnessForge behauptet keine vollständige Isolation.
