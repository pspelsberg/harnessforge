...
export function runRlm(runId: string, specs: RlmSpec[], allowedBindings: string[], token: string): Promise<RlmResult> {
  return apiJson<RlmResult>("/api/rlm/run", { method: "POST", token, body: JSON.stringify({ contract_version: "1", run_id: runId, specs, allowed_bindings: allowedBindings }) });
}
