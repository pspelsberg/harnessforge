...
export function RlmInspector({ token, spec }: { token: string; spec: RlmSpec }) {
  ...
  async function spawn() {
    ...
    try { setResult(await runRlm(spec.run_id, Array.from({ length: Math.min(maxChildren, 8) }, () => spec), spec.context.bindings, token)); }
  ...
  return <section aria-label="RLM Inspector">
    ...
    <button type="button" onClick={spawn} disabled={busy}>Sub-Agenten starten</button>
