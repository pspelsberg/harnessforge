...
from app.features.continual_refiner.contracts import AnalyzeRequest,ApplyRequest,RejectRequest,RollbackRequest
...
def router_for(service:RefinerService)->APIRouter:
 ...
 @router.post("/api/refiner/reject")
 async def reject(request:RejectRequest):
  try:return (await service.reject(request.suggestion_id,request.session_id)).model_dump(mode="json")
  except RefinerError as exc:raise HTTPException(status_code=409,detail="suggestion rejection rejected") from exc
