...
import pytest
from app.features.continual_refiner.contracts import AnalyzeRequest,ApplyRequest,Patch,RejectRequest,RollbackRequest,Trajectory
...
async def test_refiner_requires_gate_and_applies_atomic_patch_then_rolls_back(tmp_path):
 (tmp_path/"agents.md").write_text("old",encoding="utf-8"); gate_service=__import__("app.features.human_gates.service",fromlist=["HumanGateService"]).HumanGateService(tmp_path); service=RefinerService(tmp_path,gate_service); patch=patch_for("agents.md","old","new"); analysis=await service.analyze(AnalyzeRequest(trajectory=trajectory(tmp_path),candidate_patch=patch)); suggestion=analysis.suggestions[0]; preview=ActionPreview(action="refiner.apply",command=f"refiner:{suggestion.suggestion_id}:"+hashlib.sha256(json.dumps({"path":suggestion.patch.path,"expected_hash":suggestion.patch.expected_hash,"old_text":suggestion.patch.old_text,"new_text":suggestion.patch.new_text},ensure_ascii=False,sort_keys=True,separators=(",",":")).encode()).hexdigest(),diff=patch.diff,risk="high",write_targets=[patch.path]); gate=await service.gates.create(GateCreateRequest(run_id="run-1",node_id="refiner",graph_version="a"*64,workspace_realpath=str(tmp_path.resolve()),session_id="session-1",gate_class="tool_write",preview=preview));
...
async def test_refiner_rejects_prompt_injection_paths_bombs_and_gate_patch_mismatch(tmp_path):
 (tmp_path/"agents.md").write_text("old"); service=RefinerService(tmp_path,__import__("app.features.human_gates.service",fromlist=["HumanGateService"]).HumanGateService(tmp_path)); bad=patch_for("../outside","old","new");
...
@pytest.mark.asyncio
async def test_refiner_reject_is_terminal(tmp_path):
 (tmp_path/"agents.md").write_text("old"); service=RefinerService(tmp_path,__import__("app.features.human_gates.service",fromlist=["HumanGateService"]).HumanGateService(tmp_path)); analysis=await service.analyze(AnalyzeRequest(trajectory=trajectory(tmp_path),candidate_patch=patch_for("agents.md","old","new"))); suggestion=analysis.suggestions[0]; rejected=await service.reject(suggestion.suggestion_id,"session-1");assert rejected.status=="rejected"
 with pytest.raises(RefinerError):await service.reject(suggestion.suggestion_id,"session-1")
